        <footer class="footer">
          <div class="container-fluid d-flex justify-content-between">
            <div class="copyright">
              &copy;2024 - Toko Jajanan Lombok | All right reserve
            </div>
          </div>
        </footer>
      </div>
    ...
    <!--   Core JS Files   -->
    <script src="<?=base_url('assets/dashboard/js/core/jquery-3.7.1.min.js');?>"></script>
    ...
    <!-- jQuery Vector Maps -->
    <script src="<?=base_url('assets/dashboard/js/plugin/jsvectormap/jsvectormap.min.js');?>"></script>
    <script src="<?=base_url('assets/dashboard/js/plugin/jsvectormap/world.js');?>"></script>

    <!-- Sweet Alert -->
    <script src="<?=base_url('assets/dashboard/js/plugin/sweetalert/sweetalert.min.js');?>"></script>

    <!-- Kaiadmin JS -->
    <script src="<?=base_url('assets/dashboard/js/kaiadmin.min.js');?>"></script>
    ...
    <script>
      ...
      function changeToIDR(data) {
        const type = $(data).attr('id')
        var value = $(data).val().replace(/[^,\d]/g, '');
        if (value) {
          var rupiah = '';
          var valueArray = value.split(',');
          var sisa = valueArray[0].length % 3;
          var rupiah = valueArray[0].substr(0, sisa);
          var ribuan = valueArray[0].substr(sisa).match(/\d{3}/gi);
          ...
          rupiah = valueArray[1] != undefined ? rupiah + ',' + valueArray[1] : rupiah;
          $(data).val('Rp ' + rupiah);

          let valNumber = value.replace(/[^,\d]/g, '');
          $(`input[name=${type}]`).val(valNumber)
        } else {
          $(data).val('');
        }
